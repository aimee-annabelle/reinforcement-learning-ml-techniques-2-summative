"""Standalone DQN training script for the RwandaHealthEnv environment."""
...
from training.hyperparam_configs import DQN_SEARCH_SPACE
from training.train_models import build_policy_kwargs, parse_args, to_serializable

DEFAULT_TIMESTEPS = 150_000


def train_dqn(args: argparse.Namespace, configs: Sequence[Dict[str, Any]]) -> None:
    output_dir = (PROJECT_ROOT / args.output_dir).resolve() / "dqn"
    log_dir = (PROJECT_ROOT / args.log_dir).resolve() / "dqn"
    ...
    for index, base_config in enumerate(selected_configs):
        ...
        policy_kwargs = build_policy_kwargs(config, supports_ortho=False)
        total_timesteps = args.total_timesteps or int(config.pop("total_timesteps", DEFAULT_TIMESTEPS))
        eval_freq = args.eval_frequency or max(1_500, total_timesteps // 8)
        ...
        if best_model_path.exists():
            eval_model = DQN.load(best_model_path, device=args.device)
        else:
            eval_model = model
    ...
    with (output_dir / "dqn_results.json").open("w", encoding="utf-8") as handle:
        json.dump(results, handle, indent=2)

    print("Saved DQN results to", output_dir)


def main() -> None:
    args = parse_args()
    train_dqn(args, DQN_SEARCH_SPACE)
