"""Standalone A2C training script for the RwandaHealthEnv environment."""
...
from training.hyperparam_configs import A2C_SEARCH_SPACE
from training.train_models import build_policy_kwargs, parse_args, to_serializable

DEFAULT_TIMESTEPS = 180_000


def train_a2c(args: argparse.Namespace, configs: Sequence[Dict[str, Any]]) -> None:
    output_dir = (PROJECT_ROOT / args.output_dir).resolve() / "a2c"
    log_dir = (PROJECT_ROOT / args.log_dir).resolve() / "a2c"
    ...
    for index, base_config in enumerate(selected_configs):
        ...
        if best_model_path.exists():
            eval_model = A2C.load(best_model_path, device=args.device)
        else:
            eval_model = model
    ...
    with (output_dir / "a2c_results.json").open("w", encoding="utf-8") as handle:
        json.dump(results, handle, indent=2)

    print("Saved A2C results to", output_dir)


def main() -> None:
    args = parse_args()
    train_a2c(args, A2C_SEARCH_SPACE)
